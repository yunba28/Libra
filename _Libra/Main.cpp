#include "Project/Scenes/DefaultScene.hpp" // alpha

void Main()
{
	auto world = Libra::World::CreateWorld();
	{
		/// @brief Worldの初期化
		/// @brief referenceへの登録はシーンを生成する上で必須です
		world->reference = world.makeRef();

		world->add<DefaultScene>(U"DefaultScene");
	}
